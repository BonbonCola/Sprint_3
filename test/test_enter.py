...
class TestEntrance:
    ...
    def test_entrance_in_restore_form_user_exist_authorization_successful(self, driver):
        WebDriverWait(driver, 10).until(expected_conditions.visibility_of_element_located(
            (By.XPATH, locators.account_button)))  # ждем пока страничка не загрузится
        driver.find_element(By.XPATH, locators.account_button).click()  # жмем Личный кабинет в хэдере
        driver.find_element(By.XPATH, locators.restore_password_link).click()  # жмем Восстановить пароль
        driver.find_element(By.XPATH, locators.entrance_link).click()  # жмем Вход на странице восстановления пароля
        driver.find_element(By.XPATH, locators.email_input).send_keys("tacron@tqc-sheen.com")  # вводим логин
        driver.find_element(By.XPATH, locators.password_input).send_keys("123456")  # и пароль
        driver.find_element(By.XPATH, locators.entrance_button).click()  # жмем Войти
        WebDriverWait(driver, 10).until(expected_conditions.visibility_of_element_located(
            (By.XPATH, locators.account_button)))  # ждем пока страничка не загрузится
        # проверяем результат
        driver.find_element(By.XPATH, locators.account_button).click()  # кликаем на кнопку Личный кабинет
        WebDriverWait(driver, 10).until(expected_conditions.visibility_of_element_located(
            (By.XPATH, locators.profile_label)))  # ждем пока загрузится линый кабинет
